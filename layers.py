...
class Layer(object):
    ...
    def __call__(self, inputs):
        with tf.name_scope(self.name):
            ...
            if self.name=="readoutlayer_1":
                outputs_0=outputs[0]
                freatures = outputs[1]
                if self.logging:
                    tf.summary.histogram(self.name + '/outputs', outputs_0)
                return outputs_0,freatures
            else:
                if self.logging:
...
class GraphLayer(Layer):
    ...
    def __init__(self, input_dim, output_dim, placeholders, dropout=0.,
        ...
        self.steps = steps
        self.b_mask = placeholders['b_mask']
        self.upside_down=placeholders['b_mask_ud']
...
class ReadoutLayer(Layer):
    ...
    def __init__(self, input_dim, output_dim, placeholders, dropout=0.,
        ...
        self.bias = bias
        self.mask = placeholders['mask']
        if 'b_mask' in placeholders:
            self.mask=placeholders['b_mask']
        with tf.variable_scope(self.name + '_vars'):
